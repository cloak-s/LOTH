...
from Backbone import multi_resnet18_kd, VGG19_bn, multi_resnet34_kd
...
parser.add_argument('--model_name', default='MobV1', type=str)
parser.add_argument('--resume', default='', type=str, help='path to  latest checkpoint (default: None)')
parser.add_argument('--save-folder', default='BYOT/', type=str, help='folder to save the checkpoints')
...
def Train_BYOT(epoch, model, optimizer, train_loader):
    ...
    for inputs, targets in train_loader:
        ...
        for i, logit in enumerate(logits):
            ...
            if i != (len(logits)-1):
                soft_loss += Criterion_KD(logit, logits[-1].detach())

        for i in range(len(embs)-1):
            # feature_loss += Criterion_MSE(embs[i], embs[-1].detach())
            feature_loss += feature_loss_function(embs[i], embs[-1].detach())

        total_loss = (1 - args.alpha) * hard_loss + args.alpha * soft_loss + args.beta * feature_loss
    ...
    print('Exit@1: %.3f%% | Exit@2: %.3f%% | Exit@3: %.3f%% | Exit@m: %.3f%% | Exit@e: %.3f%% |'
          % (Exit_1.avg, Exit_2.avg, Exit_3.avg, acc_main.avg, acc_e.avg))
...
def Val_BYOT(model, test_loader):
    ...
    with torch.no_grad():
        ...
        print('Exit@1: %.3f%% | Exit@2: %.3f%% | Exit@3: %.3f%% | Exit@main: %.3f%% | Exit@fuse: %.3f%% |'
              % (Exit_1.avg, Exit_2.avg, Exit_3.avg, acc_main.avg, acc_e.avg))
...
if __name__ == '__main__':
    ...
    model = MobileNetV1(num_classes=num_classes, aux='BYOT').to(DEVICE)
