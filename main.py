...
from Backbone import MobileNetV2, VGG16_bn, MobileNetV1,  ShuffleNetV2, ShuffleNet, VGG19_bn, Res_18, Res_34
from models import NJK_Fusion, MECA_fusion, Norm_fusion, DECA_fusion
import argparse
import json
import time
...
parser.add_argument('--model_name', default='Res18-DECA(B)', type=str)
parser.add_argument('--resume', default='', type=str, help='path to  latest checkpoint (default: None)')
parser.add_argument('--save-folder', default='NEW/', type=str, help='folder to save the checkpoints')
...
def Train_LOTH(epoch, model, fuse_module, optimizer, optimizer_fm, train_loader):
    ...
    for inputs, targets in train_loader:
        ...
        score_att, f_logits = fuse_module(embs)
        # ensemble_logit = torch.mean(torch.bmm(score_att, torch.stack(logits, dim=1)), dim=1, keepdim=False)
        ensemble_logit = sum(logits) / len(logits)

        optimizer.zero_grad()
        optimizer_fm.zero_grad()

        Total_loss = Criterion_CE(f_logits, targets) + Criterion_KD(f_logits, ensemble_logit.detach())  # fusion_module
        for i, logit in enumerate(logits):
            Total_loss += (2 - A ** (4-i)) * Criterion_CE(logits[i], targets) \
                          + A ** (4-i) * Criterion_KD(logits[i], f_logits.detach())
...
if __name__ == '__main__':
    ...
    model = Res_18(num_classes=num_class, aux='Bottle').to(DEVICE)
    fuse_module = DECA_fusion(channel=512, layer=4, num_class=num_class).to(DEVICE)
