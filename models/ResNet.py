...
from models.Attention import SCAN_Layer, SD_Layer, DBSF_att, CS_att, MCS_att
from models.Fusion import Norm_fusion
...
class _ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100, aux=None, ):
        ...
        # self.max_pooling = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1)
        ...
        if self.aux == 'Bottle':
            ...
            self.aux_1_trans = nn.Sequential(
                ...
                self._make_layer(block, 256, num_blocks[1], stride=2),
                self._make_layer(block, 512, num_blocks[1], stride=2),
            )
            self.linear_1 = nn.Linear(512, num_classes)
...
def Res_14(num_classes, aux=None, before_relu=False):
    return _ResNet(BasicBlock, num_blocks=[1, 1, 1, 3], num_classes=num_classes, aux=aux)


def Res_18(num_classes, aux=None, before_relu=False):
    return _ResNet(BasicBlock, num_blocks=[2, 2, 2, 2], num_classes=num_classes, aux=aux)


def Res_34(num_classes, aux=None, before_relu=False):
    return _ResNet(BasicBlock, num_blocks=[3, 4, 6, 3], num_classes=num_classes, aux=aux)

def Res_50(num_classes, aux=None, before_relu=False):
    return _ResNet(Bottleneck, num_blocks=[3, 4, 6, 3], num_classes=num_classes, aux=aux)
