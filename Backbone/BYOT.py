...
def branchBottleNeck(channel_in, channel_out, kernel_size, factor=4):
    middle_channel = channel_out // factor
...
class Multi_ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return [middle1_fea, middle2_fea, middle3_fea, final_fea], [middle_output1, middle_output2, middle_output3, x]


def multi_resnet50_kd(num_classes=100):
    return Multi_ResNet(BottleneckBlock, [3, 4, 6, 3], num_classes=num_classes)


def multi_resnet18_kd(num_classes=100):
    return Multi_ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes)


def multi_resnet34_kd(num_classes=100):
...
if __name__ == '__main__':
    ...
    model = multi_resnet18_kd(num_classes=100).to(device)
    print(model)
