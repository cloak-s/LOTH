...
from Backbone import DECA_layer
from Backbone.BYOT import branchBottleNeck
from models.Auxiliary import DSC_aux
from models.Attention import SD_Layer
    ...
    def __init__(self, channels=3, num_classes=100, depth=16, bn=True, aux=None):
        ...
        if self.aux == 'Bottle':
            self.aux_1_trans = nn.Sequential(
                Bottle_aRelu(128, 256, stride=2),
                Bottle_aRelu(256, 512, stride=2),
                Bottle_aRelu(512, 512, stride=2)
            )
            ...
            self.aux_1_trans = nn.Sequential(
                Bottle_aRelu(128, 256, stride=2, Groups=True),
                Bottle_aRelu(256, 512, stride=2, Groups=True),
                Bottle_aRelu(512, 512, stride=2, Groups=True)
            )
            self.linear_1 = nn.Linear(512, num_classes)
            ...
            self.aux_1_trans = branchBottleNeck(128, 512, kernel_size=8)
            self.linear_1 = nn.Linear(512, num_classes)

            self.aux_2_trans = branchBottleNeck(256, 512, kernel_size=4)
            self.linear_2 = nn.Linear(512, num_classes)

            self.aux_3_trans = branchBottleNeck(512, 512, kernel_size=2)
            self.linear_3 = nn.Linear(512, num_classes)
...
if __name__ == '__main__':
    ...
    model = VGG19_bn(num_classes=100, aux='ECSD').cuda()
