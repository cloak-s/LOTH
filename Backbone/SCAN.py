...
class SCAN_ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=100, zero_init_residual=False, align="CONV"):
        ...
        self.attention1 = nn.Sequential(
            ...
            nn.BatchNorm2d(64* block.expansion),
            nn.Sigmoid()
        )  # 这里的注意力是否真的有用，依然是未知，有没有可能是增加了参数提升了效果
...
def SCAN_res18(num_classes=100):
    return SCAN_ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes)


def SCAN_res34(num_classes=100):
    return SCAN_ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes)


def SCAN_res50(num_classes=100):
    return SCAN_ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
...
if __name__ == '__main__':
    ...
    model = SCAN_res18(num_classes=100).to(device)
    print(model)
